...
#define LOCKS_POR_CELL 128
#define CELL_LOCK_ID 0
...
omp_lock_t **cell_lock;  // locks usados para RebuildGrid e outras funções para garantir 
                    // que as threads continuarão na mesma célula
...
void InitSim(char const *fileName, unsigned int threadnum)
{
  ...
  cell_lock = new omp_lock_t *[numCells];
  for(int i = 0; i < numCells; ++i)
  {
    assert(CELL_LOCK_ID < LOCKS_POR_CELL);
    int n = (border[i] ? LOCKS_POR_CELL : CELL_LOCK_ID+1);
    cell_lock[i] = new omp_lock_t[n];
    for(int j = 0; j < n; ++j)
...
void RebuildGridOpenMP(int tid)
{
    ...
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
      {
        ...
        for(int j = 0; j < np2; ++j)
        {
          ...
          if(border[index])
            omp_set_lock(&cell_lock[index][CELL_LOCK_ID]);
          ...
          ++cnumPars[index];
          if(border[index])
            omp_unset_lock(&cell_lock[index][CELL_LOCK_ID]);
...
void InitDensitiesAndForcesOpenMP(int tid)
{
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
...
void ComputeDensitiesOpenMP(int tid)
{
            ...
            for(int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh)
            {
              ...
              if(&neigh->p[iparNeigh % PARTICLES_PER_CELL] < &cell->p[ipar % PARTICLES_PER_CELL])
              {
                ...
                if(distSq < hSq)
                {
                  ...
                  if(border[index])
                  {
                    omp_set_lock(&cell_lock[index][ipar % LOCKS_POR_CELL]);
                    cell->density[ipar % PARTICLES_PER_CELL] += tc;
                    omp_unset_lock(&cell_lock[index][ipar % LOCKS_POR_CELL]);
                  }
                  else
                    cell->density[ipar % PARTICLES_PER_CELL] += tc;
                  ...
                  {
                    omp_set_lock(&cell_lock[indexNeigh][iparNeigh % LOCKS_POR_CELL]);
                    neigh->density[iparNeigh % PARTICLES_PER_CELL] += tc;
                    omp_unset_lock(&cell_lock[indexNeigh][iparNeigh % LOCKS_POR_CELL]);
                  }
                  else
                    neigh->density[iparNeigh % PARTICLES_PER_CELL] += tc;
                }
...
void ComputeDensities2OpenMP(int tid)
{
...
void ComputeForcesOpenMP(int tid)
{
  int neighCells[3*3*3];
...
#if 0
void ProcessCollisionsOpenMP(int tid)
{
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
...
void ProcessCollisions2OpenMP(int tid)
{
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
  {
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
...
void AdvanceParticlesOpenMP(int tid)
{
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
...
void *AdvanceFramesOpenMP(int threadnum, int framenum)
{
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  AdvanceFramesOpenMP(threadnum, framenum);
